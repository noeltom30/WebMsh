...
function addToGroup(group, gmshGeo, fallbackGeo, color, fallbackPos) {
  const geo = gmshGeo || fallbackGeo
  const wire = new THREE.LineSegments(new THREE.EdgesGeometry(geo), new THREE.LineBasicMaterial({ color }))
  if (!gmshGeo && fallbackPos) wire.position.set(...fallbackPos)
  group.add(wire)
}
...
function Workspace({ projectId, user, onLogout }) {
  ...
  useEffect(() => {
    ...
    geoms.forEach(rec => {
      ...
      if (rec.type === 'box') {
        ...
        addToGroup(group, gmshGeo, new THREE.BoxGeometry(w, h, d), 0x4b8fea, [ox + w / 2, oz + h / 2, oy + d / 2])
        return
      }
      if (rec.type === 'sphere') {
        const r = Number(p.radius) || 1
        addToGroup(group, gmshGeo, new THREE.SphereGeometry(r, 24, 16), 0xf5c542, [Number(p.center_x) || 0, Number(p.center_z) || 0, Number(p.center_y) || 0])
        return
      }
      if (rec.type === 'cylinder') {
        const r = Number(p.radius) || 1, h = Number(p.height) || 1
        addToGroup(group, gmshGeo, new THREE.CylinderGeometry(r, r, h, 24, 1), 0x5ad35a, [Number(p.base_x) || 0, (Number(p.base_z) || 0) + h / 2, Number(p.base_y) || 0])
        return
      }

      // Sketches — flat teal wireframe
      if (SKETCH_TYPES.has(rec.type)) {
        if (!gmshGeo) return
        group.add(new THREE.LineSegments(new THREE.EdgesGeometry(gmshGeo), new THREE.LineBasicMaterial({ color: 0x2dd4bf })))
        // Filled surface with low opacity
        const mesh = new THREE.Mesh(gmshGeo, new THREE.MeshStandardMaterial({ color: 0x2dd4bf, transparent: true, opacity: 0.18, side: THREE.DoubleSide }))
        group.add(mesh)
        return
      }

      // Extrude / revolve / upload — orange/indigo wireframe
      if (['extrude', 'revolve', 'upload', 'cad'].includes(rec.type)) {
        if (!gmshGeo) return
        const color = rec.type === 'upload' || rec.type === 'cad' ? 0x7ad4ff : 0xf97316
        group.add(new THREE.LineSegments(new THREE.EdgesGeometry(gmshGeo), new THREE.LineBasicMaterial({ color })))
        return
      }
    })
  }, [geoms])
  ...
  const loadWorkspaceData = async (message = '') => {
    ...
    try {
      setLoading(true)
      const [healthData, infoData, projectData] = await Promise.all([api.health(), api.info(), api.getProject(numericProjectId)])
      ...
      if (err?.status === 401) { setLastAction('Session expired.'); await onLogout?.(); navigate('/auth', { replace: true }); return }
      if (err?.status === 404) { setLastAction('Project not found.'); return }
      setLastAction(err?.body?.detail || 'Workspace data could not be loaded.')
  ...
  const handleBox = async () => {
    try {
      const g = await api.createBox(numericProjectId, {
        width: Number(boxParams.width), height: Number(boxParams.height), depth: Number(boxParams.depth),
        origin_x: parseFloat(boxParams.origin_x) || 0, origin_y: parseFloat(boxParams.origin_y) || 0, origin_z: parseFloat(boxParams.origin_z) || 0,
        mesh_settings: buildSettings(),
      })
      await loadWorkspaceData(`Created box #${g.id}`)
    } catch (e) { setLastAction(e?.body?.detail || 'Failed to create box.') }
  }
  ...
  const handleSphere = async () => {
    try {
      const g = await api.createSphere(numericProjectId, {
        ...
        center_x: parseFloat(sphereParams.center_x) || 0, center_y: parseFloat(sphereParams.center_y) || 0, center_z: parseFloat(sphereParams.center_z) || 0,
        mesh_settings: buildSettings(),
      })
      await loadWorkspaceData(`Created sphere #${g.id}`)
    } catch (e) { setLastAction(e?.body?.detail || 'Failed to create sphere.') }
  }

  const handleCylinder = async () => {
    try {
      const g = await api.createCylinder(numericProjectId, {
        radius: Number(cylParams.radius), height: Number(cylParams.height),
        base_x: parseFloat(cylParams.base_x) || 0, base_y: parseFloat(cylParams.base_y) || 0, base_z: parseFloat(cylParams.base_z) || 0,
        mesh_settings: buildSettings(),
      })
      await loadWorkspaceData(`Created cylinder #${g.id}`)
    } catch (e) { setLastAction(e?.body?.detail || 'Failed to create cylinder.') }
  }

  const handleCadUpload = async () => {
    if (!cadFile) { setLastAction('Choose a STEP, IGES, BREP, STL, VTK, or MSH file first.'); return }
    try {
      const g = await api.uploadCAD(numericProjectId, cadFile)
      setCadFile(null)
      await loadWorkspaceData(`Uploaded geometry #${g.id}`)
    } catch (e) { setLastAction(e?.body?.detail || 'Failed to upload CAD or mesh.') }
  }

  const handleDeleteGeom = async (id) => {
    try { await api.deleteGeometry(numericProjectId, id); await loadWorkspaceData(`Deleted geometry #${id}`) }
    catch (e) { setLastAction(e?.body?.detail || 'Failed to delete geometry.') }
  }
